...
Items.schema = new SimpleSchema([BaseSchema, {
    ...
    createdFromMessageId: {type:String, optional:true}
}]);
...
Items.methods.addItem = new ValidatedMethod({
    ...
    run({description, projectId, type, subType, assignee, milestoneId, status, createdFromMessageId}) {
        ...
        var createdFromMessage;
        if(Meteor.isServer && createdFromMessageId) {
            createdFromMessage = ServerMessages.findOne(createdFromMessageId);
            if(createdFromMessage == null) {
                throw Meteor.Error(
                    'Items.methods.addItem.created-from-message-doesnt-exist',
                    'Error adding item - invalid created-from message: ' + createdFromMessageId
                );
            }
        }
        ...
        if(Meteor.isServer) {
            if(createdFromMessage) {
                var msgLink = "[this](/project/" + createdFromMessage.projectId + "/start-message/" + createdFromMessage.seq + "?scrollTop=true&selectStartMessage=true) message";
                Ols.Activity.addActivity({
                    projectId,
                    itemId,
                    longDescription: "created " + subType + " **" + Ols.Links.generateItemKeyLink(item.projectId, item._id, item.projectKey, seq) + ": " + description + "** from " + msgLink,
                    shortDescription: "created this " + item.subType + " from " + msgLink
                });
            } else {
                Ols.Activity.addActivity({
                    projectId,
                    itemId,
                    longDescription: " added " + subType + " **" + Ols.Links.generateItemKeyLink(item.projectId, item._id, item.projectKey, seq) + ": " + description + "**",
