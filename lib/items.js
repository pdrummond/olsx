            ...
            if (item.subType == Ols.Item.INFO_SUBTYPE_QUESTION) {
                if (Ols.QuestionStatus.isClosed(status)) {
                    Ols.Activity.addActivity({
                        ...
                        itemId: item._id,
                        longDescription: "closed " + item.subType + " " + item.projectKey + "-" + item.seq + " as **" + Ols.QuestionStatus.getStatusLabel(status) + "**",
                    ...
                    Ols.Activity.addActivity({
                        projectId: item.projectId,
                        itemId: item._id,
                        longDescription: "changed status of " + item.subType + " " + item.projectKey + "-" + item.seq + " to **" + Ols.QuestionStatus.getStatusLabel(status) + "**",
                        shortDescription: "set status to **" + Ols.QuestionStatus.getStatusLabel(status) + "**"
                    });
                }
            } else {
                if (Ols.Status.isDone(status)) {
                    Ols.Activity.addActivity({
                        projectId: item.projectId,
                        itemId: item._id,
                        longDescription: " closed " + item.subType + " " + item.projectKey + "-" + item.seq + " as **" + Ols.Status.getStatusLabel(status) + "**",
                    ...
                    Ols.Activity.addActivity({
                        projectId: item.projectId,
                        itemId: item._id,
                        longDescription: " changed status of " + item.subType + " " + item.projectKey + "-" + item.seq + " to **" + Ols.Status.getStatusLabel(status) + "**",
                        shortDescription: " changed status to **" + Ols.Status.getStatusLabel(status) + "**"
                    });
                }
            }
    ...
    run({itemId, priority}) {
        ...
        if(Meteor.isServer) {
            Ols.Activity.addActivity({
                projectId: item.projectId,
                itemId: item._id,
                longDescription: "changed priority of " + item.subType + " " + item.projectKey + "-" + item.seq + " to **" + priority + "**",
    ...
    run({itemId, type, subType}) {
        ...
        if(Meteor.isServer) {
            Ols.Activity.addActivity({
                projectId: item.projectId,
                itemId: item._id,
                longDescription: "changed type of " + item.subType + " " + item.projectKey + "-" + item.seq + " to **" + subType + "**",
    ...
    run({itemId}) {
        ...
        if(Meteor.isServer) {
            Ols.Activity.addActivity({
                projectId: item.projectId,
                itemId: item._id,
                longDescription: "removed priority value from " + item.subType + " " + item.projectKey + "-" + item.seq,
    ...
    run({itemId}) {
        ...
        if(Meteor.isServer) {
            Ols.Activity.addActivity({
                projectId: item.projectId,
                itemId: item._id,
                longDescription: "archived " + item.subType + " " + item.projectKey + "-" + item.seq,
    ...
    run({itemId}) {
        ...
        if(Meteor.isServer) {
            Ols.Activity.addActivity({
                projectId: item.projectId,
                itemId: item._id,
                longDescription: "restored " + item.subType + " " + item.projectKey + "-" + item.seq,
    ...
    run({itemId, milestoneId}) {
        ...
        if(Meteor.isServer) {
            Ols.Activity.addActivity({
                projectId: item.projectId,
                itemId: item._id,
                longDescription: "added " + item.subType + " " + item.projectKey + "-" + item.seq + " to milestone " + milestone.title,
    ...
    run({itemId}) {
        ...
        if(Meteor.isServer) {
            Ols.Activity.addActivity({
                projectId: item.projectId,
                itemId: item._id,
                longDescription: "moved " + item.subType + " " + item.projectKey + "-" + item.seq + " to the backlog",
    ...
    run({itemId}) {
        ...
        if(Meteor.isServer) {
            Ols.Activity.addActivity({
                projectId: item.projectId,
                itemId: item._id,
                longDescription: "deleted " + item.subType + " " + item.projectKey + "-" + item.seq,
    ...
    run({itemId, title}) {
        ...
        if(Meteor.isServer) {
            Ols.Activity.addActivity({
                projectId: item.projectId,
                itemId: item._id,
                longDescription: "renamed " + item.subType + " " + item.projectKey + "-" + item.seq + " to **" + title + "**",
    ...
    run({itemId, description}) {
        ...
        if(Meteor.isServer) {
            Ols.Activity.addActivity({
                projectId: item.projectId,
                itemId: item._id,
                longDescription: "set description for " + item.subType + " " + item.projectKey + "-" + item.seq + " to \n\n>" + truncatedDesc,
    ...
    run({itemId, assignee}) {
        ...
        if(Meteor.isServer) {
            Ols.Activity.addActivity({
                projectId: item.projectId,
                itemId: item._id,
                longDescription: "assigned " + item.subType + " " + item.projectKey + "-" + item.seq + " to **" + assignee + "**",
    ...
    run({itemId}) {
        ...
        if(Meteor.isServer) {
            Ols.Activity.addActivity({
                projectId: item.projectId,
                itemId: item._id,
                longDescription: "unassigned **" + assignee + "** from " + item.subType + " " + item.projectKey + "-" + item.seq,
