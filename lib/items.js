    ...
    run({itemId, status}) {
        ...
        if(item.subType == Ols.Item.INFO_SUBTYPE_QUESTION) {
            if (Ols.QuestionStatus.isClosed(status)) {
                Ols.Message.systemSuccessMessage(item.projectId, Meteor.user().username + " closed " + item.subType + " " + item.projectKey + "-" + item.seq + " as " + Ols.QuestionStatus.getStatusLabel(status));
            } else {
                Ols.Message.systemSuccessMessage(item.projectId, Meteor.user().username + " changed status of " + item.subType + " " + item.projectKey + "-" + item.seq + " to " + Ols.QuestionStatus.getStatusLabel(status));
            }
        } else {
            if (Ols.Status.isDone(status)) {
                Ols.Message.systemSuccessMessage(item.projectId, Meteor.user().username + " closed " + item.subType + " " + item.projectKey + "-" + item.seq + " as " + Ols.Status.getStatusLabel(status));
            } else {
                Ols.Message.systemSuccessMessage(item.projectId, Meteor.user().username + " changed status of " + item.subType + " " + item.projectKey + "-" + item.seq + " to " + Ols.Status.getStatusLabel(status));
            }
        }
