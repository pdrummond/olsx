...
Items.methods.addItem = new ValidatedMethod({
    ...
    run({description, projectId, type, subType}) {
        ...
        if(Meteor.isServer) {
          ...
          console.log('-- saving activity message for item key ' + key);
          var message = Ols.Message.systemSuccessMessage(projectId,
              Meteor.user().username + " added " + subType + " " + key + ": " + description);
        ...
        item._id = itemId;
        console.log("-- item " + item.key + " saved as item " + item._id + ".");
