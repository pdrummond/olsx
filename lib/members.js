...
Members.methods.addMember = new ValidatedMethod({
    ...
    run({emailOrUsername, conversationId, role}) {
        ...
        //Inform the user of their new membership to a conversation
...
Members.methods.removeMember = new ValidatedMethod({
    ...
    run({memberId}) {
        ...
        if(memberToRemove.role == Ols.ROLE_ADMIN) {
            var numAdmins = Members.find({role:Ols.ROLE_ADMIN}).count();
            if(numAdmins == 1) {
               throw new Meteor.Error("Members.methods.removeMember.last-admin", "Conversation must have at least one admin");
            }
        }
