...
Projects.methods.addProject = new ValidatedMethod({
    ...
    run({title, key}) {
        ...
        if(Projects.findOne({key}) != null) {
            throw new Meteor.Error("Projects.methods.addProject.key-is-not-unique");
        }
        ...
        var project = {
            title: title,
            type: Ols.Project.PROJECT_TYPE_STANDARD,
            key: key,
            createdAt: now,
            updatedAt: now,
            createdBy: userId,
...
Projects.methods.setTitle = new ValidatedMethod({
    ...
    run({projectId, title}) {
        ...
        var sysMsg = Ols.Message.systemSuccessMessage(projectId, Meteor.user().username + " renamed this project to '" + title + "'");
        if(Meteor.isServer) {
            Meteor.call('addActivityMessage', (Meteor.user().username + " renamed this project to '" + title + "'"), projectId, sysMsg._id, sysMsg.seq, projectId);
        }
    }
});
...
Projects.methods.setProjectCurrentRelease = new ValidatedMethod({
    ...
    run({projectId, releaseId}) {
        ...
        Ols.Message.systemSuccessMessage(projectId, Meteor.user().username + " set current release to " + release.projectKey + "-" + release.seq + ": " + release.title);
    }
});
...
Projects.methods.removeProjectCurrentRelease = new ValidatedMethod({
    ...
    run({projectId, releaseId}) {
        ...
        Ols.Message.systemSuccessMessage(projectId, Meteor.user().username + " set " + release.projectKey + "-" + release.seq + " to not be the current release any more");
    }
});
...
Projects.methods.setProjectNextRelease = new ValidatedMethod({
    ...
    run({projectId, releaseId}) {
        ...
        Ols.Message.systemSuccessMessage(projectId, Meteor.user().username + " set next release to " + release.projectKey + "-" + release.seq + ": " + release.title);
    }
});
...
Projects.methods.removeProjectNextRelease = new ValidatedMethod({
    ...
    run({projectId, releaseId}) {
        ...
        Ols.Message.systemSuccessMessage(projectId, Meteor.user().username + " set " + release.projectKey + "-" + release.seq + " to not be the next release any more");
    }
});
...
Projects.methods.setTheme = new ValidatedMethod({
    ...
    run({projectId, theme}) {
        ...
        var sysMsg = Ols.Message.systemSuccessMessage(projectId, Meteor.user().username + " changed the project theme to '" + theme + "'");
        if(Meteor.isServer) {
            Meteor.call('addActivityMessage', (Meteor.user().username + " changed the project theme to '" + theme + "'"), projectId, sysMsg._id, sysMsg.seq, projectId);
        }
    }
});
